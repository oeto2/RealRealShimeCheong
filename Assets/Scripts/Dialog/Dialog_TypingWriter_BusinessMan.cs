...
public class Dialog_TypingWriter_BusinessMan : MonoBehaviour
{
    ...
    IEnumerator TextPractice()
    {
        ...
        if (EventManager.instance.eventEndCheck.muckEvent_End == true)
        {
            ...
            yield return StartCoroutine(ItemClueChat(npcDatabaseScr.NPC_01[153].npc_name, npcDatabaseScr.NPC_01[153].comment, true));

            //짚신을 사간 청이 단서 획득
            ObjectManager.instance.GetClue(2016);
            yield return StartCoroutine(ItemClueChat(npcDatabaseScr.NPC_01[154].npc_name, npcDatabaseScr.NPC_01[154].comment));
        }
